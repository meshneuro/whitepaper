...
- \*\*Intelligence density.\*\* $ I = \log N \times \bar{\beta} \times C $ where $N$ is active robots/agents, $ \bar{\beta} $ useful message rate, and $ C $ coherence (fraction of high-quality CTV/A).
- \*\*Verification overhead.\*\* $ \psi \in [0,1) $, multiplier $ \chi = \psi/(1-\psi) $.
- **PoI tail bound.** $ P\_{\mathrm{FA}} \approx \exp{-2q(1/2-\theta)^2} \Rightarrow q \ge \ln(1/\varepsilon)/\[2(1/2-\theta)^2] $.
...
${\mathrm{cCOMP(on!-!robot)}, \mathrm{iCTVs}, \mathrm{nROBOT\ slice}, \mathrm{optional\ nDATA/nDATA!-!R}, \mathrm{optional\ nENERGY}}$.\
...
For each tranche $ r $:\
$
\text{NAV}_r = PV(\text{cashflows}_r) - \text{liabilities}_r - \text{reserves}_r,\qquad
\sum_r \text{issued}_r \cdot \text{unit\_value}_r \le \sum_r \text{NAV}_r (1-h_r).
$
...
+ T\,\bar{\varepsilon}\,(e^{\bar{\varepsilon}}-1),
$
...
$
\mathrm{Royalty}(X) = \sum_i \phi_i(X) \cdot r_i(t),\quad 
...
- **PoI correctness:** $ P_{\mathrm{FA}} \le \varepsilon \Rightarrow q \ge \ln(1/\varepsilon)/[2(1/2-\theta)^2] $.  
- **CBC safety:** choose $ u $ to minimise $ \|u-u_{\mathrm{nom}}\|^2 $ s.t. $ \partial h/\partial x \cdot (f+g u) + \alpha h(x) \ge 0 $ and actuator/contact limits.  
...
Dual-descent:
$
p_R(t{+}1)=\max(0,p_R+\eta_p[D_R-\mathcal{U}_R]),\quad
p_v(t{+}1)=\max(0,p_v+\eta_v[\psi-\psi^\*]).
$
...
S = S_{\mathrm{comp},R} (1+\chi) + S_{\mathrm{body}},\ \chi=\psi/(1-\psi).
$
...
$1{,}200 \times [6.2(1{,}050\cdot0.0030) + 6.2\cdot0.15 + 6.2\cdot0.02 + 0.28] \approx 1{,}200 \times 20.86 \approx 25{,}032\ $NEURO/day$.  
...
1. **PoI quorum:** $ q \ge \ln(1/\varepsilon)/\[2(1/2-\theta)^2] $.
2. \*\*Overhead:\*\* $ \chi=\psi/(1-\psi) $, $ S = S\_{\mathrm{comp},R}(1+\chi) + S\_{\mathrm{body}} $.
3. **Queueing:** $ W \approx \rho\_R/(\mu\_R-\lambda\_R),\ \mu\_R=1/\mathbb{E}\[S\_{\mathrm{comp},R}] $.
4. **CBC:** $ \partial h/\partial x \cdot (f+g u) + \alpha h(x) \ge 0 $.
5. **Intelligence density:** $ I=\log N \times \bar{\beta} \times C $.
6. **Throughput:** $ V\_{\mathrm{emb}} \propto I \times \mathcal{U}*R \times (1-P*{\mathrm{unsafe}}) $.
7. **DP composition:** $ \varepsilon\_{\mathrm{total}} \le \sqrt{2T\ln(1/\delta)},\bar{\varepsilon} + T,\bar{\varepsilon}(e^{\bar{\varepsilon}}-1) $.
8. **Data price:** $ P(D)=\alpha \mathbb{E}\[\Delta I]+\beta \kappa\_{\mathrm{cov}}+\gamma \rho\_{\mathrm{rare}}-\lambda\_{\mathrm{risk}}\mathcal{R} $.
9. **Emissions:** $ E\_{\mathrm{day}}=P\_{\mathrm{dev}} h,\ \mathrm{CO2e}=E\_{\mathrm{day}}\kappa\_{\mathrm{CO2}} $.
10. \*\*Emissions (token):\*\* $ E_t=E_0\cdot 2^{-t/H} $ (10-year pool).
